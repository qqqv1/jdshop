...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/order-commons.css" source="widget"/>
...
<form id="orderForm" class="hide" action="order/addOrder" method="post">
    <%--<input type="hidden" name="paymentType" value="1"/>--%>
    <input type="hidden" name="order" value="${order}">
    <%--<c:forEach items="${order.items}" var="map">
        <c:set var="totalPrice" value="${order.total}"/>
        <c:set var="orderItem" value="${map.value}"/>
        <c:set var="product" value="${orderItem.product}"/>
        <input type="hidden" name="orderItems[${status.index}].itemId" value="${product.pid}"/>
        <input type="hidden" name="orderItems[${status.index}].num" value="${orderItem.count }"/>
        <input type="hidden" name="orderItems[${status.index}].price" value="${product.shopPrice}"/>
        <input type="hidden" name="orderItems[${status.index}].totalFee" value="${orderItem.subTotal}"/>
        <input type="hidden" name="orderItems[${status.index}].title" value="${product.pname}"/>
        <input type="hidden" name="orderItems[${status.index}].picPath" value="${product.pimage}"/>
    </c:forEach>--%>
                    ...
                    <div id="shipAndSkuInfo">
                        <div id="payShipAndSkuInfo">
                            ...
                            <div class="step-tit">
                                <h3>送货清单</h3>
                                <div class="extra-r">
                                    <a href="shoppingcart" id="cartRetureUrl" class="return-edit ftx-05">返回修改购物车</a>
                                </div>
                            </div>
                                            ...
                                            <div class="goods-suit goods-last">
                                                <c:forEach items="${order.items }" var="map">
                                                    ...
                                                    <div class="goods-item goods-item-extra">
                                                        <div class="p-img">
                                                            <a target="_blank" href="product-portal-info?pid=${product.pid}">
                                                                <img src="${product.pimage}" alt="">
                                                            </a>
                                                        </div>
                                                        <div class="goods-msg">
                                                            <div class="p-name">
                                                                <a href="product-portal-info?pid=${product.pid}" target="_blank">
                                                                        ${product.pname }
                                                                </a>
                                                            </div>
                                                            <div class="p-price">
                                                                ...
                                                                <strong>￥<fmt:formatNumber
                                                                        ...
                                                                        minFractionDigits="2"
                                                                        value="${product.shopPrice}"/></strong>
                                                                <!--增加预售金额显示 end-->
                                                                <span class="ml20"> x${orderItem.count} </span>
