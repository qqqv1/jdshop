    ...
    $(function () {
        $("#categoryListDg").bootstrapTable({
            ...
            columns: [[
                ...
                {
                    field: 'parentId', title: '父级目录', formatter: function(v, r, i) {
                    $.ajax({
                       url:'category/'+v,
                       dataType:'json',
                       success:function (data) {
                           $('#categoryListDg').find('tr:eq(' + (i+1) + ') td:eq(3)').text(data.cname);
                       }
                    });
                },sortable : true
                },
                ...
                {title:'操作',formatter: function actionFormatter(value, row, index) {
                    ...
                    'click #mod': function(e, value, row, index) {
                        //修改操作
                        location.href='category2-edit?cid='+row.cid;
                        return false;
                    },
...
//                    type:"POST",
    ...
    function edit() {
        ...
        if(selRow.length > 0){
            ...
            cid=selRow[0].cid;
            location.href='category2-edit?cid='+cid;
        }else{
            alert('请选取要编辑的数据行！');
            return false;
        }
